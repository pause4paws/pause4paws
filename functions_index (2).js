...
const CHAT_ID      = "@pause4paws_ae";        // public channel — for pet alerts & digests
const ADMIN_CHAT_ID = "8638960270";           // your private Telegram — for payment notifications
...
// ══════════════════════════════════════════════════════════════════════════
// 5. PENDING PAYMENT NOTIFICATION (NEW)
//    Triggers when app writes a new pendingPayment doc (user taps "I've paid")
//    Sends admin a Telegram message with one-tap activation link
// ══════════════════════════════════════════════════════════════════════════
exports.notifyPendingPayment = functions.firestore
  .document("pendingPayments/{uid}")
  .onCreate(async (snap, context) => {
    const uid  = context.params.uid;
    const data = snap.data();
    if (!data || data.notified) return null;

    const BOT_TOKEN = BOT_TOKEN_FN();
    if (!BOT_TOKEN) return null;

    const APP_URL_ACTIVATE = `https://pause4paws.github.io/pause4paws/?activate=${uid}`;
    const name   = data.name  || "Unknown";
    const email  = data.email || "—";
    const amount = data.amount || 38;
    const type   = data.type === "featured" ? "Featured listing (25 AED/week)" : "Lifetime membership (38 AED)";

    const msg =
      `💳 <b>NEW PAYMENT — action needed</b>\n\n` +
      `👤 <b>${name}</b>\n` +
      `📧 ${email}\n` +
      `💰 ${amount} AED — ${type}\n\n` +
      `<b>Steps:</b>\n` +
      `1️⃣ Open your Wio app and confirm you received ${amount} AED from ${name}\n` +
      `2️⃣ Once confirmed, tap this link to activate instantly:\n\n` +
      `✅ <a href="${APP_URL_ACTIVATE}">Activate ${name}'s account →</a>\n\n` +
      `<i>Link works when you're logged in as admin on the app.</i>`;

    try {
      // Send PRIVATELY to admin — not to the public channel
      ...
      console.log(`[P4P] Payment notification sent privately for ${name} (${uid})`);
    } catch (err) {
      console.error("[P4P] Payment notify error:", err.message);
    }
    return null;
  });
...
// ══════════════════════════════════════════════════════════════════════════
// 6. MORNING ADMIN SUMMARY (NEW)
//    Runs at 8:00am Dubai — sends admin a summary of pending payments
//    so nothing gets missed overnight
// ══════════════════════════════════════════════════════════════════════════
exports.adminMorningSummary = functions.pubsub
  .schedule("0 4 * * *")   // 08:00 Dubai = 04:00 UTC
  .timeZone("Asia/Dubai")
  .onRun(async () => {
    const BOT_TOKEN = BOT_TOKEN_FN();
    if (!BOT_TOKEN) return null;

    // Find all unactivated pending payments
    const pendingSnap = await db.collection("pendingPayments")
      .where("activated", "==", false)
      .get();

    if (pendingSnap.empty) {
      console.log("[P4P] Morning summary: no pending payments");
      return null;
    }

    const pending = pendingSnap.docs.map(d => ({ uid: d.id, ...d.data() }));

    // Also get overall stats
    const [usersSnap, petsSnap] = await Promise.all([
      db.collection("users").where("paid", "==", true).get(),
      db.collection("pets").where("status", "==", "lost").get()
    ]);

    let msg = `☀️ <b>Good morning — Pause4Paws daily summary</b>\n\n`;

    // Pending payments section
    msg += `💳 <b>${pending.length} payment${pending.length > 1 ? 's' : ''} waiting for activation:</b>\n`;
    pending.forEach((p, i) => {
      const APP_URL_ACTIVATE = `https://pause4paws.github.io/pause4paws/?activate=${p.uid}`;
      const timeAgo = p.createdAt
        ? Math.floor((Date.now() - p.createdAt._seconds * 1000) / 3600000) + 'h ago'
        : 'recently';
      msg += `${i + 1}. <b>${p.name || 'Unknown'}</b> · ${p.amount || 38} AED · ${timeAgo}\n`;
      msg += `   ✅ <a href="${APP_URL_ACTIVATE}">Activate →</a>\n`;
    });

    // Stats
    msg += `\n📊 <b>Stats:</b>\n`;
    msg += `👥 ${usersSnap.size} paid members\n`;
    msg += `🔴 ${petsSnap.size} pets currently missing\n`;
    msg += `\n🔗 <a href="https://pause4paws.github.io/pause4paws/">Open app →</a>`;

    try {
      // Send privately to admin only
      await tgPost("sendMessage", { chat_id: ADMIN_CHAT_ID, text: msg, parse_mode: "HTML" });
      console.log(`[P4P] Morning summary sent: ${pending.length} pending payments`);
    } catch (err) {
      console.error("[P4P] Morning summary error:", err.message);
    }
    return null;
  });
